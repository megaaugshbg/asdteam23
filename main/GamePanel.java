...
public class GamePanel extends JPanel {
    ...
    public GamePanel(GameFrame frame, int numPlayers) {
        ...
        for (Player p : players) {
            p.setVisualPosition(start.x, start.y);
        }
    ...
    private void setupUI() {
        ...
        int uiWidth = 260;
        int uiX = sc.width - uiWidth - 20;

        backBtn = new JButton("Quit Game");
        backBtn.setBounds(30, 30, 150, 45);
        ...
        statusPanel.setBounds(uiX, 50, uiWidth, 60);
        statusPanel.setBackground(new Color(100, 149, 237));
        statusPanel.setBorder(BorderFactory.createLineBorder(Color.BLACK, 2));

        statusLabel = new JLabel("Turn: Player 1");
        ...
        scoreTitle.setBounds(uiX, 120, uiWidth, 20);
        scoreTitle.setFont(new Font("Arial", Font.BOLD, 14));
        scoreTitle.setHorizontalAlignment(SwingConstants.CENTER);
        add(scoreTitle);

        scoreArea = new JTextArea();
        scoreArea.setBounds(uiX, 145, uiWidth, 140);
        scoreArea.setEditable(false);
        scoreArea.setFont(new Font("Monospaced", Font.BOLD, 13));
        scoreArea.setBackground(new Color(240, 248, 255));
        scoreArea.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
        add(scoreArea);

        rollDiceBtn = new JButton("ROLL DICE");
        rollDiceBtn.setBounds(uiX, 300, uiWidth, 60);
        rollDiceBtn.setBackground(new Color(255, 193, 7));
        rollDiceBtn.setForeground(Color.BLACK);
        rollDiceBtn.setFont(new Font("Arial", Font.BOLD, 18));
        rollDiceBtn.addActionListener(e -> rollDiceAction());
        add(rollDiceBtn);
        ...
        legend.setBackground(new Color(255, 255, 255, 220));
        add(legend);
    }
    ...
    private void updateStatus() {
        ...
        for (Player pl : players) {
            sb.append(" ")
                    .append(String.format("%-8s", pl.name))
                    .append("| ")
                    .append(pl.score)
                    .append(" pts\n");
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        int offX = (int) ((getWidth() - (720.0 * scale)) / 2);
        int offY = (int) ((getHeight() - (680.0 * scale)) / 2);
        ...
        for (Node n : gameMap.nodes) {
            for (Node nb : n.neighbors) {
                if (n.id < nb.id && Math.abs(n.id - nb.id) == 1) {
                    ...
                    g2d.drawLine(n.x, n.y, nb.x, nb.y);
                }
            }
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            g2d.fillOval(px - 10, py + 5, 20, 10);

            g2d.setColor(p.color);
            g2d.fillOval(px - 10, py - 10, 20, 20);

            if (i == currentPlayerIndex) {
                g2d.setColor(Color.YELLOW);
                g2d.setStroke(new BasicStroke(2));
                g2d.drawOval(px - 10, py - 10, 20, 20);
            }
        }
